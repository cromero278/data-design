<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8">
		<title>Kristen Shepard Persona</title>
	</head>
	<body>
	<p>
		<h1>Customer Persona</h1>
			<img src="kristenShepard.jpg" alt="Kristen Photo">
			<p>
			<strong>Name:</strong> Kristen Shepard
			<br>
			<strong>Age:</strong> 21
			<br>
			<strong>Profession:</strong> College Student
			<br>
			<strong>Hobbies:</strong> Spending time with sorority sisters, attending college football games, long distance running
			<br>
			<strong>Technology:</strong> iPhone10, 13 inch 2017 MacBook Air (OS Sierra)
			<br>
			<strong>Frequency of Use</strong>
			Very familiar and frequent user of technology, especially social media platforms.
			Prefers mobile device over laptop use.
			Tends to use apps more than web browser.
			<br>
			<strong>Technological Needs:</strong> Selling platform that is easy to use on a mobile device. Secure payment method.
			<br>
			<strong>Frustrations:</strong> Not comfortable selling items face to face.
			Current platforms such as Craigslist only reach a local vicinity, offer no way to accept payment online.
		</p>
		<h2>User Story</h2>
		<p>
			Kristen sells used North Face jacket online.
		</p>
		<h2>Use Case</h2>
			<strong>Title:</strong> Kristen sells North Face jacket on Poshmark.
			<br>
			<strong>Description:</strong> Kristen uses mobile phone to find/use selling platform for clothing items.
			<br>
			<strong>Usage Pre-conditions:</strong> Must have mobile phone capable of downloading/displaying apps and taking photos.
			<br>
			<strong>Usage Post-conditions:</strong> The application sends Kristen offers from potential buyers.
			<h2>Interaction Flow</h2>
		<ul>
			<li>Kristen downloads Poshmark app on her iPhone and opens the application once downloaded</li>
			<li>Poshmark prompts Kristen to sign in or create an account and asks if she would like to receive notifications from the app on her phone</li>
			<li>Kristen does not have an account, so proceeds to create one, entering first and last name, email, username password and gender. She opts to receive app notifications to her phone</li>
			<li>Poshmark returns page asking for additional optional information</li>
			<li>Kristen chooses to bypass entering the additional optional information</li>
			<li>Poshmark sends Kristen to the homepage of the app</li>
			<li>Kristen clicks the sell icon in the center of the bottom navigation bar</li>
			<li>Poshmark opens the camera on Kristen's phone and prompts her to take a picture of the item she is trying to sell</li>
			<li>Kristen takes a photo of the North Face jacket</li>
			<li>Poshmark adds the photo to the 'Listing Details' page and requests additional information, including 'What are you selling?', 'Describe your item' 'Category', 'Quantity', 'Size', 'Price'</li>
			<li>Kristen inputs all required information and clicks 'Next' on the top right of the screen</li>
			<li>Poshmark opens a new page, asking Kristen if she would like optionally like to share the listing on her social media accounts, and prompts her to post the listing</li>
			<li>Kristen chooses not to share the listing on her social media and proceeds to post the listing</li>
			<li>Poshmark makes the listing live and opens the listing page as it will appear to other users</li>
			<li>Kristen reviews her listing, ensuring all the information is correct and proceeds to close the app</li>
		</ul>
	</body>
</html>